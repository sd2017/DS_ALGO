...
void Disp(ZGGZ tp[], int
          n) /*显示数组tp[]中存储的记录，内容为employee结构中定义的内容*/
{
    int i;
...
int Add(ZGGZ tp[], int n)
{
    ...
    while (1) { /*一次可输入多条记录，直至输入职工编号为0的记录才结束添加操作*/
        while (1) { /*输入职工编号，保证该编号没有被使用，若输入编号为0，则退出添加记录操作*/
            stringinput(num, 10,
                        "input number(press '0'return menu):"); /*格式化输入编号并检验*/
            flag = 0;
...
void Qur(ZGGZ tp[], int n)
{
    ...
    if (select == 1) { /*按编号查询*/
        ...
        p = Locate(tp, n, searchinput,
                   "num"); /*在数组tp中查找编号为searchinput值的元素，并返回该数组元素的下标值*/
...
void Modify(ZGGZ tp[], int n)
{
    ...
    stringinput(findmess, 10,
                "input the existing employee number:"); /*输入并检验该编号*/
...
{
    char ch, num[10],
         s[10]; /*s[]保存插入点位置之前的编号,num[]保存输入的新记录的编号*/
    ZGGZ newinfo;
...
void Tongji(ZGGZ tp[], int n)
{
    ...
    while (i < n) {
        if (tp[i].sfgz >= 10000) {
            count10000++;    /*实发工资>10000*/
            i = i + 1;
            continue;
        }

        if (tp[i].sfgz >= 5000)  {
            count5000++;    /*5000<=实发工资<10000*/
            i = i + 1;
            continue;
        }

        if (tp[i].sfgz >= 2000)  {
            count2000++;    /*2000<=实发工资<5000*/
            i = i + 1;
            continue;
        }
...
void Save(ZGGZ tp[], int n)
{
    ...
    for (i = 0; i < n; i++) {
        if (fwrite(&tp[i], sizeof(ZGGZ), 1,
                   fp) == 1) { /*每次写一条记录或一个结构数组元素至文件*/
            continue;
        } else {
            break;
        }
...
void main()
{
    ...
    while (!feof(fp)) {
        if (fread(&gz[count], sizeof(ZGGZ), 1,
                  fp) == 1) { /*一次从文件中读取一条职工工资记录*/
            count++;
        }
    }
