...
void showHelp()
{
    ...
    outtextxy(45, 80,
              "      3 Use PAGEUP key to enlarge it, and PAGEDOWN key to shrink it.");
    ...
    setcolor(WHITE);
    outtextxy(45, 120,
              "           1 Press left button to start until to right corner.");
    ...
    outtextxy(45, 185,
              "        2 Use PAGEUP key to enlarge it, and PAGEDOWN key to shrink it.");
    ...
    setcolor(WHITE);
    outtextxy(45, 205,
              "        Press left button to start, and right button to end.");
            ...
            while (1) {
                ...
                if (key == SPACE) {
                    ...
                    if (lineStartx >= 10 && lineStarty >= 40 && lineEndx <= 620 &&
                        ...
                        /*计算旋转30度后的起点坐标*/
                        lineStartx = (temStartx - Centx) * cos(pi / 6) - (temStarty - Centy) * sin(
                                         pi / 6) + Centx;
                        lineEndx = (temEndx - Centx) * cos(pi / 6) - (temEndy - Centy) * sin(
                                       pi / 6) + Centx;

                        /*计算旋转30度后的终点坐标*/
                        lineStarty = (temStartx - Centx) * sin(pi / 6) + (temStarty - Centy) * cos(
                                         pi / 6) + Centy;
                        lineEndy = (temEndx - Centx) * sin(pi / 6) + (temEndy - Centy) * cos(
                                       pi / 6) + Centy;
...
float berFunction(int i, int n, double t)
{
    ...
    return  factorial(n) / (factorial(i) * factorial(n - i)) * pow(t,
            i) * pow(1 - t, n - i);
...
void main()
{
    ...
    while (1) {
        ...
        while (!((mouseStatus(&x1, &y1) == NO_PRESSED) && x1 > 240 && x1 < 295 &&
                 y1 > 1 && y1 < 25)) {
            /*单击退出按钮*/
            if ((mouseStatus(&x1, &y1) == NO_PRESSED) && x1 > 570 && x1 < 625 && y1 > 1 &&
                y1 < 25) {
                exit(0);
            }
