...
int Curbox_x = Sys_x + Begin_boxs_x * BSIZE,
    Curbox_y = Sys_y; /*x,y是保存方块的当前坐标的*/
int flag_newbox = false; /*是否要产生新方块的标记0*/
int speed = 0;          /*下落速度*/
int score = 0;          /*总分*/
int speed_step = 30;     /*每等级所需要分数*/
void interrupt(*oldtimer)(
    void); /* 指向原来时钟中断处理过程入口的中断处理函数指针 */
...
int DelFullRow(int y)
{
    /*该行游戏板往下移一行*/
    int n, top =
        0;             /*top保存的是当前最高点,出现一行全空就表示为最点了,移动是到最高点结束*/
    register m, totoal;
    ...
    for (n = y; n >= 0; n--) { /*从当前行往上看*/
        ...
        for (m = 0; m < Horizontal_boxs; m++) {
            ...
            if (Table_board[n][m].var != Table_board[n -
                    1][m].var) { /*上行不等于下行就把上行传给下行 xor关系*/
            ...
            for (m = 0; m < Horizontal_boxs; m++) { /*水平的方向*/
                if (Table_board[n][m].var) {
                    setfillstyle(SOLID_FILL,
                                 Table_board[n][m].color);    /*Table_board[n][m].color*/
                } else {
                    setfillstyle(SOLID_FILL, BgColor);
                }
    ...
    for (n = 0; n < 4; n++) {
        for (m = 0; m < 4; m++) {
            if (((shapes[current_box_numb].box[n / 2]) & mask)) {
                ...
                Table_board[t_boardy + n][t_boardx + m].color =
                    shapes[current_box_numb].color; /*这里设置游戏板*/
            }
    ...
    for (n = 0; n < 4; n++) {
        for (m = 0; m < 4; m++) { /*看最左边四个单元*/
            if (((shapes[box_numb].box[n / 2]) &
                ...
                line(x + m * BSIZE, y + n * BSIZE + BSIZE, x + m * BSIZE + BSIZE,
                     y + n * BSIZE + BSIZE);
                line(x + m * BSIZE + BSIZE, y + n * BSIZE, x + m * BSIZE + BSIZE,
                     y + n * BSIZE + BSIZE);
            }
        ...
        for (n = 0; n < 4; n++) {
            for (m = 0; m < 4; m++) { /*看最左边四个单元*/
                if ((shapes[box_numb].box[n / 2]) &
                    ...
                    if ((x + BSIZE * m) < Sys_x) {
                        return (false);    /*碰到最左边了*/
                    } else if (Table_board[t_boardy + n][t_boardx +
                                                         m].var) { /*左移一个方块后，此4*4的区域与游戏板有冲突*/
                        return (false);
                    }
                }
...
void main()
{
    ...
    while (1) {
        ...
        if (flag_newbox) {
            ...
            if (!MoveAble(Curbox_x, Curbox_y, current_box_numb,
                          MoveDown)) { /*刚一开始,游戏结束*/
                show_box(Curbox_x, Curbox_y, current_box_numb, shapes[current_box_numb].color);
                GameOver = 1;
            } else {
