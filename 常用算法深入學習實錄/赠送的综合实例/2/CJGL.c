...
void Disp(Link
          l)  /*显示单链表l中存储的学生记录，内容为student结构中定义的内容*/
{
    Node* p;
...
void Add(Link l)
{
    ...
    while (1) { /*一次可输入多条记录，直至输入学号为0的记录结点添加操作*/
        while (1) { /*输入学号，保证该学号没有被使用，若输入学号为0，则退出添加记录操作*/
            ...
            stringinput(num, 10,
                        "input number(press '0'return menu):"); /*格式化输入学号并检验*/
            flag = 0;
        ...
        p->data.cgrade =
            numberinput("C language Score[0-100]:"); /*输入并检验分数，分数必须在0－100之间*/
        p->data.mgrade =
            numberinput("Math Score[0-100]:"); /*输入并检验分数，分数必须在0－100之间*/
        p->data.egrade =
            numberinput("English Score[0-100]:"); /*输入并检验分数，分数必须在0－100之间*/
...
void Qur(Link l) /*按学号或姓名，查询学生记录*/
{
    ...
    if (select == 1) { /*按学号查询*/
        ...
        p = Locate(l, searchinput,
                   "num"); /*在l中查找学号为searchinput值的节点，并返回节点的指针*/
...
void Modify(Link l)
{
    ...
    stringinput(findmess, 10,
                "input the existing student number:"); /*输入并检验该学号*/
...
void Insert(Link l)
{
    ...
    char ch, num[10],
         s[10]; /*s[]保存插入点位置之前的学号,num[]保存输入的新记录的学号*/
    int flag = 0;
    v = l->next;
    system("cls");
    Disp(l);
    ...
    newinfo->data.total = newinfo->data.egrade + newinfo->data.cgrade +
                          newinfo->data.mgrade;
...
void Sort(Link l)
{
    ...
    while (p) { /*p!=NULL*/
        ...
        if (rr->next ==
            NULL) { /*若新链表ll中的所有节点的总分值都比p->data.total大时，就将p所指节点加入链表尾部*/
            rr->next = s;
...
void Save(Link l)
{
    ...
    while (p) {
        if (fwrite(p, sizeof(Node), 1,
                   fp) == 1) { /*每次写一条记录或一个节点信息至文件*/
            p = p->next;
            count++;
        } else {
            break;
...
void main()
{
    ...
    r = l;
    fp = fopen("C:\\student",
               "ab+"); /*以追加方式打开一个二进制文件，可读可写，若此文件不存在，会创建此文件*/
