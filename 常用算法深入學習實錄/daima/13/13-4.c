...
int Check(char chessboard[][8], int moves[][8],
          char player);   //检查一方是否有地方可下子
void PlayStep(char chessboard[][8], int row, int col,
              char player); //在指定位置下子
void AutoPlayStep(char chessboard[][8], int moves[][8],
                  char player);   //计算机在思考下子
int GetMaxScore(char chessboard[][8], char player); //获取分数
int BestPlay(char chessboard[][8], int moves[][8],
             char player);    //最优下子
int main()
{
        ...
        do {
            if (player == 1) { //若是游戏者下棋(下白子)
                ...
                if (Check(chessboard, isDown, 1)) { //检查是否可下白子
                 ...
                 SkipPlay < 2); //下子数量小于64 且无效下子的次数小于2
...
int Check(char chessboard[][8], int isDown[][8],
          char player)   //检查某一方是否还有下子的地方
{
    ...
    for (row = 0; row < 8;
         row++) {  //循环判断棋盘中哪些单元格可以下子
        for (col = 0; col < 8; col++) {
            if (chessboard[row][col] !=
                0) { //若棋盘上对应位置不为空(表示已经有子)
                continue;    //继续处理下一个单元格
            }
            ...
            for (rowdelta = -1; rowdelta <= 1; rowdelta++) { //循环检查上下行
                for (coldelta = -1; coldelta <= 1; coldelta++) { //循环检查左右列
                    if (row + rowdelta < 0 || row + rowdelta >= 8
                        ...
                        || (rowdelta == 0 &&
                            coldelta == 0)) { //检查若坐标超过棋盘 或为当前单元格
                        continue;    //继续循环
                    }

                    if (chessboard[row + rowdelta][col + coldelta] ==
                        opponent) { //若(row,col)四周有对手下的子
                        ...
                        while (1) { //对对手下子为起始点，向四周查找自己方的棋子，以攻击对方棋子
                            ...
                            if (chessboard[x][y] ==
                                myplayer) { //若对应位置下的子是当前棋手的
                                isDown[row][col] =
                                    1;   //设置移动数组中对应位置为1 (该位置可下子，形成向对手进攻的棋形)
                                iStep++;    //累加可下子的位置数量
                                break;
                            }
                        }
              ...
              char player)  //在指定位置下子
{
    ...
    for (rowdelta = -1; rowdelta <= 1;
         rowdelta++) { //检查所下子四周的棋子
        for (coldelta = -1; coldelta <= 1; coldelta++) {
            if (row + rowdelta < 0 || row + rowdelta >= 8 || col + coldelta < 0
                || col + coldelta >= 8 || (rowdelta == 0 &&
                                           coldelta == 0)) { //若坐标超过棋盘界限
                continue;    //继续下一位置
            }
...
int GetMaxScore(char chessboard[][8], char player)  //获取分数
{
    ...
    for (row = 0; row < 8; row++) {  //循环
        for (col = 0; col < 8; col++) {
            Score -= chessboard[row][col] ==
                     opponent; //若棋盘对应位置是对手下的棋子，从总分中减1
            Score += chessboard[row][col] ==
                     myplayer; //若棋盘对应位置是我方的棋子，总分中加1分
        }
    }

    return Score;    //返回分数
}
int BestPlay(char chessboard[][8], int isDown[][8],
             char player)    //获取最佳下子位置
{
    int row, col, i, j;
    ...
    for (row = 0; row < 8; row++) {  //循环检查每个单元格
        for (col = 0; col < 8; col++) {
            ...
            for (i = 0; i < 8;
                 i++)    //复制棋盘各单元格下子的状态到临时数组
                for (j = 0; j < 8; j++) {
                    chessboard1[i][j] = chessboard[i][j];
                }

            PlayStep(chessboard1, row, col,
                     player);   //在临时数组中的指定行列下子
            Score = GetMaxScore(chessboard1, player);  //获取下子后可得到的分数

            if (MaxScore <
                Score) { //若原方案得到的分数小于本次下子的分数
...
void AutoPlayStep(char chessboard[][8], int isDown[][8],
                  char player)   //计算机自动下子
{
    int row, col, row1, col1, i, j;
    int Score = 0, MinScore =
                    100;   //对方可下子提到的分数和最小分数
    ...
    for (row = 0; row < 8; row++) {  //循环检查棋盘每个单元格
        for (col = 0; col < 8; col++) {
            ...
            PlayStep(chessboard1, row, col,
                     player);   //试着在临时棋盘中的一个位子下子
            Check(chessboard1, isDown1, opponent); //检查对手是否有地方可下子
            Score = BestPlay(chessboard1, isDown1,
                             opponent);  //获得临时棋盘中对方下子的得分情况
