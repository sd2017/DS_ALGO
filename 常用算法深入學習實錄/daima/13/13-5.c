...
int Pop(Stack* s, MazeNode*
        mn) // 若栈不空删除栈//顶元素用e返回并返回OK，否则返回ERROR
{
    if (s->top != -1) { //栈不为空
        *mn = s->base[s->top];
        s->top--;
...
int MarkerNoPass(MazeType* maze,
                 Coordinate pos)    // 曾走过但不是通路标记并返回OK
{
    maze->grid[pos.row][pos.column] = '3';   // "3"表示曾走过但不通
    return 1;
}
int MazePath(MazeType* maze, Coordinate start,
             Coordinate end)//从迷宫maze的入口到出口查找路径
{
    Stack S; //定义栈
    Coordinate pos;
    ...
    do {
        if (Pass(maze, pos)) { //若指定位置可通过
            ...
            if (pos.row == end.row &&
                pos.column == end.column) { //若当前位置是出口坐标
            ...
            if (!StackEmpty(&S)) { //若栈不为空(之前有走过的位置)
                ...
                while (e.di == 4 &&
                       !StackEmpty(&S)) { //上一步4个方向都探测完,且栈不为空
