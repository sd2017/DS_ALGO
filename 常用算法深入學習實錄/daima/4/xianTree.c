...
int BinTreeAddNode(ThreadBinTree* bt, ThreadBinTree* node,
                   int n) //添加数据到二叉树
...
int BinTreeIsEmpty(ThreadBinTree*
                   bt) //检查二叉树是否为空，为空则返回1,否则返回0
{
    if (bt) {
        return 0;
    } else {
...
ThreadBinTree* BinTreeFind(ThreadBinTree* bt,
                           DATA data) //在二叉树中查找值为data的结点
{
    ThreadBinTree* p;
...
void BinTreeClear(ThreadBinTree*
                  bt) // 清空二叉树，使之变为一棵空树
{
    if (bt) {
...
void BinTree_DLR(ThreadBinTree* bt,
                 void (*oper)(ThreadBinTree* p)) //先序遍历
{
    if (bt) { //树不为空，则执行如下操作
        oper(bt); //处理结点的数据
...
void BinTree_LDR(ThreadBinTree* bt,
                 void(*oper)(ThreadBinTree* p)) //中序遍历
{
    if (bt) { //树不为空，则执行如下操作
...
void BinTree_LRD(ThreadBinTree* bt,
                 void (*oper)(ThreadBinTree* p)) //后序遍历
{
    if (bt) {
...
void BinTree_Level(ThreadBinTree* bt,
                   void (*oper)(ThreadBinTree* p)) //按层遍历
{
    ThreadBinTree* p;
...
ThreadBinTree* BinTreeNext_LDR(ThreadBinTree* bt) //求指定结点的后继
{
    ...
    if (bt->rflag == Thread) { //若当前结点的右子树为空
        ...
        while (nextnode->lflag ==
               SubTree) { //循环处理所有左子树不为空的结点
...
ThreadBinTree* BinTreePrevious_LDR(ThreadBinTree*
                                   bt) //求指定结点的前驱
{
...
void ThreadBinTree_LDR(ThreadBinTree* bt,
                       void (*oper)(ThreadBinTree* p)) //遍历中序线索二叉树
{
    if (bt) { //二叉树不为空
