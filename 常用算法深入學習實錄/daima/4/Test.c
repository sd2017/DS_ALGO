    ...
    for (i = 1; i <= n;
        ...
        if (!ht[i].parent) { //父结点为空(结点的parent=0)
            ...
            if (ht2 == NULL) { //结点指针2为空
                ...
                if (ht1->weight >
                    ht2->weight) { //比较两个结点的权重，使ht1指向的结点权重小
                    t = ht2;
                    ht2 = ht1;
                    ht1 = t;
                }
            ...
            if (ht1 && ht2) { //若ht1、ht2两个指针都有效
                if (ht[i].weight <=
                    ht1->weight) { //第i个结点权重小于ht1指向的结点
                    ht2 = ht1; //ht2保存ht1，因为这时ht1指向的结点成为第2小的
                    ht1 = ht + i; //ht1指向第i个结点
                } else if (ht[i].weight <
                           ht2->weight) { //若第i个结点权重小于ht2指向的结点
                    ht2 = ht + i; //ht2指向第i个结点
                }
            }
        }
...
void CreateTree(HuffmanTree* ht, int n, int* w)
{
    ...
    for (i = n + 1; i <= m; ++i) { //逐个计算非叶结点，创建Huffman树
        SelectNode(ht, i - 1, &bt1,
                   &bt2); //从1~i-1个结点选择parent结点为0,权重最小的两个结点
        ht[bt1].parent = i;
        ht[bt2].parent = i;
        ht[i].left = bt1;
        ht[i].right = bt2;
...
void HuffmanCoding(HuffmanTree* ht, int n, HuffmanCode* hc)
{
    ...
    for (i = 1; i <= n; i++) {
        ...
        hc[i - 1] = (char*)malloc(sizeof(char) * (n -
                                  start)); //分配保存编码的内存
...
//hc为Huffman编码表 ,alphabet为对应的字母表,str为需要转换的字符串,code返回转换的结果
{
    ...
    while (str[i]) {
        ...
        strcpy(code + len,
               hc[j]); //将对应字母的Huffman编码复制到code指定位置
...
//ht为Huffman二叉树,m为字符数量,alphabet为对应的字母表,str为需要转换的字符串,decode返回转换的结果
{
    ...
    while (code[position]) { //字符串未结束
        for (i = m; ht[i].left &&
             ht[i].right;
             position++) { //在Huffman树中查找左右子树为空 ，以构造一个Huffman编码
