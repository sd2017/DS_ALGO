...
typedef struct edgeNode {
    ...
    int weight; //权值
    struct edgeNode*
        next; //指向下一个顶点指针 (当前顶点和指向的下一顶点构成一条边)
} EdgeNode; //邻接表边结构
...
void Createlin(ListGraph* G); //生成图的邻接表
void Outlin(ListGraph* G); //输出邻接表

void Createlin(ListGraph* G)  //构造邻接表结构图
{
    int i, weight;
    int start, end;
    EdgeNode* s;
