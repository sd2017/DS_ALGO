...
char* toPosfix(char* infix)     // 求后序表达式
{
    ...
    if (!(buf = (char*)malloc(sizeof(char) * length *
                              2))) { //保存后序表达式字符串
        printf("内存分配失败!\n");
        exit(0);
    }
...
double eval(char* postfix)      //计算表达式的值
{
    ...
    int i, length, top = 0, dec = 0,
                   flag; //dec为0表示整数,为1表示小数,flag=1表示有数据需入栈
    char token;
    ...
    for (i = 0; i < length; i++) {
        ...
        case '/':
            ...
            stack[top - 1] = calc(stack[top - 1], token,
                                  stack[top]); //取出栈栈前两个元素进行运算，结果保存到栈中
            top--; //修改栈顶指针
            dec = 0; //先设为整数
            flag = 0; //下一步操作不将数入栈
            break;
