...
int hash[haxi_LEN] = {0}; //哈希表，初始化为0
void Inserthaxi(int hash[], int m,
                int data) //将关键字data插入哈希表hash中
{
    int i;
    i = data % 13; //计算哈希地址
...
{
    int key, i, pos;
    Createhaxi(hash, haxi_LEN, data, TABLE_LEN); //调用函数创建哈希表
    printf("哈希表中各元素的值:");
    ...
    printf("输入查找关键字:");
    scanf("%ld", &key);
    pos = haxisou(hash, haxi_LEN, key); //调用函数在哈希表中查找
