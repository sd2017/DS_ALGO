...
{
    printf("%d/%d %02d:%02d:%02d\n", gm_date.tm_mon, gm_date.tm_mday,
           gm_date.tm_hour + 8, gm_date.tm_min, gm_date.tm_sec);
}
...
void OutPark(SeqStack* s,
             CarChainQueue*
             q) //车出站出当时过道上的情况放在过道上直接进入车站
{
    struct tm* gm_date;
    time_t seconds;
    ...
    if (StackIsEmpty(s)) { //如果车位中没有车辆停放
        ...
        if (pos > 0 && pos <= s->top + 1) { //输入车位号正确
            for (i = s->top + 1; i > pos;
                 i--) { //在将pos车位之后停的车放入临时栈，以使pos车位的车出来
                car = StackPop(s); //出栈
            ...
            car.length = mktime(&car.outtime) - mktime(
                             &car.intime); //停车场中停放时间
            car.expense = (car.length / 3600 + 1) * PRICE; //费用
            PrintRate(
                &car);   //输出车出站时的情况---进入时间，出站时间，原来位置，花的费用等
            ...
            while (!StackIsFull(s) &&
                   !ChainQueueIsEmpty(q)) { //如果车位未满, 且过道上还有车
...
int main()
{
    ...
    do {
        ...
        printf("提示：本停车场有%d个车位，车位停满后的车辆将停放在过道上。\n",
               MAXNUM);
        printf("本停车场按时间计费，收费标准:%.2f元/小时,过道上不收费。\n\n",
               PRICE);
        printf("\n选择操作(0~3):");
        fflush(stdin);
        choice = getchar();
