#include <string.h>
ChainListType* ChainListAddEnd(ChainListType* head,
                               DATA data) //添加结点到链表结尾
{
...
ChainListType* ChainListInsert(ChainListType* head, char* findkey,
                               DATA data) //插入结点到链表指定位置
{
    ChainListType* node, *node1;

    if (!(node = (ChainListType*)malloc(sizeof(
                                            ChainListType)))) { //分配保存结点的内容
        printf("为保存结点数据申请内存失败！\n");
        return 0;  //分配内存失败
    }
