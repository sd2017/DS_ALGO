        ...
        for (j = 1; j <= m; j++) {
            if (chessboard[i][j] != -1) {  //若该位置不是马的控制点
                if ((chessboard[i - 1][j] != -1) &&
                    (chessboard[i][j - 1] != -1)) { //上面和左边也不是马的控制点
                    chessboard[i][j] = chessboard[i - 1][j] + chessboard[i][j -
                                       1];    //将上面和左边的路径进行累加
                }
                ...
                if ((chessboard[i - 1][j] == -1) &&
                    (chessboard[i][j - 1] == -1)) { //若上面和左边都有马的控制点
